...
kmfunc <- function(data = rsdata, impdata = imprsdata, time, event, eventname, novar = NULL,
  ...
  p <- ggplot(plot_data, aes(x = time / 365, y = estimate, color = strata)) +
    ...
    annotate("text",
      ...
      hjust = 0, size = 6, colour = c("black", global_cols[4:1])
    ) +
    annotate("text",
      x = textadjx + 1.65, y = c(textadjy, textadjy - 0.1, textadjy - 0.2, textadjy - 0.3, textadjy - 0.4), label = cox_crude,
      hjust = 0.5, size = 6, colour = c("black", global_cols[4:1])
    ) +
    annotate("text",
      x = textadjx + 3.15, y = c(textadjy, textadjy - 0.1, textadjy - 0.2, textadjy - 0.3, textadjy - 0.4), label = cox_adj,
      hjust = 0.5, size = 6, colour = c("black", global_cols[4:1])
    )
  ...
  t <- ggplot(table_data, aes(y = strata, x = time / 365)) +
    geom_text(aes(label = n.risk), size = 6, colour = rep(global_cols[4:1], each = 6)) +
    # scale_x_continuous(breaks = seq(0, endfollow, 1), limits = c(0, endfollow), expand = c(0, 0)) +
    ...
    theme(
      ...
      axis.text.y = element_text(hjust = 0, colour = global_cols[1:4])
    )
