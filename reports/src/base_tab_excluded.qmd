...
# special fix for NT-proBNP by ekg
misntekg <- rsdataexclude %>%
  filter(!is.na(shf_ekg)) %>%
  mutate(
    shf_ekg = case_match(shf_ekg, c("Sinus", "PM/Other") ~ "Sinus/PM/Other", .default = shf_ekg),
    mis = factor(if_else(is.na(shf_ntprobnp), 1, 0), levels = 0:1)
  ) %>%
  group_by(shf_ekg) %>%
  count(mis, .drop = F) %>%
...
tab <- tab %>%
  mutate(pmis = case_when(
    str_detect(var, "shf_ntprobnp_af") ~ misntekg %>% filter(shf_ekg == "Atrial fibrillation") %>% pull(pmis),
    str_detect(var, "shf_ntprobnp_sinus") ~ misntekg %>% filter(shf_ekg == "Sinus/PM/Other") %>% pull(pmis),
    TRUE ~ pmis
  ))
