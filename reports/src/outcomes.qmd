...
outcomesfunc <- function(data, impdata, novar) {
  ...
  default_kable(outall) %>%
    ...
    footnote(
      general_title = "",
      general = "Abbreviations: HF: Heart failure; CV: Cardiovascular; py: person-years; CI: Confidence interval"
    )
}
```
...
outcomesfunc(data = rsdata %>% filter(shf_ef_cat == "HFrEF"), impdata = impdataref, novar = "shf_ef_cat")
```
...
outcomesfunc(data = rsdata %>% filter(shf_ef_cat == "HFmrEF"), impdata = impdatamref, novar = "shf_ef_cat")
```
...
outcomesfunc(data = rsdata %>% filter(shf_ef_cat == "HFpEF"), impdata = impdatapef, novar = "shf_ef_cat")
```
...
#| cache.comments: false
#| dependson: outcomesfunc
#| tbl-cap: "Association between outcomes and QoL - Out-patient"

impdataout <- mice::filter(imprsdata, rsdata %>% pull(shf_location) == "Out-patient")

outcomesfunc(data = rsdata %>% filter(shf_location == "Out-patient"), impdata = impdataout, novar = "shf_location")
```
...
#| cache.comments: false
#| dependson: outcomesfunc
#| tbl-cap: "Association between outcomes and QoL - In-patient"

impdatain <- mice::filter(imprsdata, rsdata %>% pull(shf_location) == "In-patient")

outcomesfunc(data = rsdata %>% filter(shf_location == "In-patient"), impdata = impdatain, novar = "shf_location")
```
