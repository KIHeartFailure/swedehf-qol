...
#| fig-cap: "Independent associations between baseline characteristics and QoL"
#| fig-width: 10
#| fig-height: 6
...
p1 <- ggplot(adj %>% filter(qol == "0-25"), aes(x = or, y = var2, color = cols)) +
  ...
  scale_color_manual(values = global_cols[c(3, 2, 1)]) +
  theme_classic() +
  theme(
...
t_or1 <- ggplot(adj %>% filter(qol == "0-25")) +
...
t_or2 <- ggplot(adj %>% filter(qol == "26-50")) +
  geom_text(aes(y = var2, x = 1, label = orci), size = size_use / .pt * 0.8) +
  theme_classic() +
  theme(
    text = element_text(size = size_use),
...
t_or3 <- ggplot(adj %>% filter(qol == "51-75")) +
  geom_text(aes(y = var2, x = 1, label = orci), size = size_use / .pt * 0.8) +
  theme_classic() +
  theme(
    text = element_text(size = size_use),
...
pall <- wrap_elements(labs) + wrap_elements(pall3) + wrap_elements(pall2) + wrap_elements(pall1) +
  plot_layout(nrow = 1, widths = c(1.5, 2, 2, 2))
